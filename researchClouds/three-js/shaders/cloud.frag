...
//
//// Import Simplex noise function (or use a Perlin noise function if you prefer)
//
//uniform float time;
...
//        total += snoise(vec3(uv * frequency, time)) * amplitude;
//        frequency *= 2.0;
//        amplitude *= 0.5;
//    }
//    return total;
...
//
//    // Get the cloud density using fBm noise
//    float cloudDensity = smoothstep(cloudThreshold, cloudThreshold + cloudSoftness, fBm(uv * scale, time));
//
//    // Set the output color of the fragment shader
//    fragColor = vec4(vec3(cloudDensity), 1.0);
//}
//